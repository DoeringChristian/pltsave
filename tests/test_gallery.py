...
class TestPairwiseData:
    ...
    def test_stairs(self, temp_file):
        ...
        ax.stairs(values, fill=True, alpha=0.5, color='orange', edgecolor='black', linewidth=2)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
...
class TestStatisticalDistributions:
    ...
    def test_errorbar(self, temp_file):
        ...
        ax.errorbar(x, y, yerr=yerr, xerr=xerr, fmt='o-', capsize=5, capthick=2,
                   ecolor='red', color='blue', linewidth=2)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
    ...
    def test_eventplot(self, temp_file):
        ...
        fig, ax = plt.subplots()
        np.random.seed(42)
        data = [np.random.gamma(2, size=50), np.random.gamma(4, size=50), np.random.gamma(6, size=50)]

        ax.eventplot(data, colors=['red', 'green', 'blue'], lineoffsets=[1, 2, 3], linelengths=0.5)
        ax.set_xlabel('Value')
        ax.set_ylabel('Group')
...
class TestGriddedData:
    ...
    def test_streamplot(self, temp_file):
        ...
        sp = ax.streamplot(X, Y, U, V, color=np.sqrt(U**2 + V**2), cmap='autumn', density=1.5)
