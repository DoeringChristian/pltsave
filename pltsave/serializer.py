...
def _serialize_patch(patch: Patch) -> Dict[str, Any]:
    ...
    if isinstance(patch, Rectangle):
        ...
        data.update(
            {
                "values": (
                    patch._values.tolist()
                    if hasattr(patch._values, "tolist")
                    else list(patch._values)
                ),
                "edges": (
                    patch._edges.tolist()
                    if hasattr(patch._edges, "tolist")
                    else list(patch._edges)
                ),
                ...
                data.update(
                    {
                        ...
                        "codes": (
                            path.codes.tolist() if path.codes is not None else None
                        ),
                    }
                )
            except:
...
def _serialize_collection(collection) -> Dict[str, Any]:
    ...
    if HAS_3D and isinstance(collection, Path3DCollection):
        ...
        data["sizes"] = (
            collection.get_sizes().tolist() if len(collection.get_sizes()) > 0 else []
        )
        ...
        data["linewidths"] = (
            collection.get_linewidths().tolist()
    ...
    # Get offsets for PathCollection (scatter plots) - regular 2D
        ...
        try:
            data["lineoffsets"] = (
                float(collection.get_lineoffsets()[0])
                if hasattr(collection, "get_lineoffsets")
                else 0
            )
        except:
            data["lineoffsets"] = 0
        ...
        try:
            data["linelengths"] = (
                float(collection.get_linelengths()[0])
                if hasattr(collection, "get_linelengths")
                else 1
            )
        except:
            data["linelengths"] = 1
...
def _serialize_contour(contour: ContourSet) -> Dict[str, Any]:
    ...
    data = {
        ...
        "levels": (
            contour.levels.tolist()
            if hasattr(contour.levels, "tolist")
            else list(contour.levels)
        ),
        "alpha": contour.alpha,
...
def _serialize_legend(legend: Legend) -> Dict[str, Any]:
    ...
    data = {
        ...
        "facecolor": _serialize_color(
            legend.get_frame().get_facecolor() if legend.get_frame() else None
        ),
        "edgecolor": _serialize_color(
            legend.get_frame().get_edgecolor() if legend.get_frame() else None
        ),
...
def _serialize_axes(ax: Axes) -> Dict[str, Any]:
    ...
    try:
        grid_visible = (
            len(ax.xaxis.get_gridlines()) > 0
            and ax.xaxis.get_gridlines()[0].get_visible()
        ) or (
            len(ax.yaxis.get_gridlines()) > 0
            and ax.yaxis.get_gridlines()[0].get_visible()
        )
    except:
        grid_visible = False
